...
export default function KontaktPage() {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f0f2f5', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', color: '#0f172a' }}>

      {/* Navbar */}
      <nav style={{ padding: '0 40px', height: '68px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid #f8fafc' }}>
        <Link href="/" style={{ fontSize: '20px', fontWeight: '900', color: '#6366f1', textDecoration: 'none', letterSpacing: '-0.5px' }}>2Fast4Sale</Link>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: '48px', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            {[
              ...
              <div key={item.label} style={{ backgroundColor: '#ffffff', border: '1px solid #f8fafc', borderRadius: '18px', padding: '22px', display: 'flex', gap: '16px', alignItems: 'flex-start' }}>
                ...
                <div>
                  ...
                  {item.href ? (
                    <a href={item.href} style={{ fontSize: '15px', fontWeight: '700', color: '#0f172a', textDecoration: 'none' }}>{item.value}</a>
                  ) : (
                    <div style={{ fontSize: '15px', fontWeight: '700', color: '#0f172a', whiteSpace: 'pre-line' }}>{item.value}</div>
                  )}
                </div>
              </div>
            ))}
            ...
            <div style={{ background: 'linear-gradient(135deg, rgba(245,158,11,0.12), rgba(139,92,246,0.08))', border: '1px solid rgba(245,158,11,0.25)', borderRadius: '18px', padding: '24px' }}>
              <div style={{ fontSize: '14px', fontWeight: '800', color: '#d97706', marginBottom: '8px' }}>Enterprise-Lösung gesucht?</div>
          ...
          {/* Kontaktformular */}
          <div style={{ backgroundColor: '#ffffff', border: '1px solid #f8fafc', borderRadius: '24px', padding: '40px' }}>
            {sent ? (
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                <h3 style={{ fontSize: '18px', fontWeight: '900', color: '#0f172a', margin: '0 0 4px 0' }}>Nachricht schreiben</h3>
